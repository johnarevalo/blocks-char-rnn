...
def sample(x_curr, x_mask, states_values, fprop, temperature=1.0):
    '''
    ...
    '''
    activations = fprop(x_curr, x_mask, *states_values)
...
if __name__ == '__main__':
    ...
    x_curr_mask = numpy.ones_like(x_curr)
    ...
    fprop = theano.function([x, x_mask] + states_as_params, activations + [y_hat[-1, -1, :]], givens=zip(initial_states, states_as_params))

    states_values = [initial.get_value() for initial in initial_states]
    sys.stdout.write('Starting sampling\n' + primetext)
    for _ in range(args.length):
        idx, states_values = sample(x_curr, x_curr_mask, states_values, fprop, args.temperature)
